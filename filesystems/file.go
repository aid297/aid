...
func (my *File) Read(attrs ...OperationAttributer) ([]byte, error) {
	...
	if file, err = os.OpenFile(
		my.FullPath,
		operations.NewTernary(
			operations.TrueFn(func() int { return fileOperation.Flag }),
			operations.FalseFn(func() int { return DefaultReadFlag }),
		).GetByValue(fileOperation.Flag != 0),
		operations.NewTernary(
			operations.TrueFn(func() os.FileMode { return fileOperation.Mode }),
			operations.FalseFn(func() os.FileMode { return os.FileMode(0777) }),
...
func (my *File) CopyTo(isRel bool, dstPaths ...string) Filesystem {
	...
	if dstDir := NewDir(operations.NewTernary(operations.TrueValue(Rel(dstPaths...)), operations.FalseValue(Abs(dstPaths...))).GetByValue(isRel)).Up(); dstDir.GetError() != nil {
	...
	newPath := NewFile(
		operations.NewTernary(
			operations.TrueFn(func() PathAttributer { return Rel(dstPaths...) }),
			operations.FalseFn(func() PathAttributer { return Abs(dstPaths...) }),
		).GetByValue(isRel),
	)
