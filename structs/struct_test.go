...
func TestCover(t *testing.T) {
	...
	src = structs.NewStruct(src).CoverBySkip(dst).Value()
...
func TestCoverNotModifyInput(t *testing.T) {
	...
	structs.NewStruct(src).CoverBySkip(dst)
...
func TestCoverBySkip(t *testing.T) {
	...
	src = structs.NewStruct(src).CoverBySkip(dst, "Name", "Tags").Value()
	...
	t.Run("不传黑名单时覆盖全部", func(t *testing.T) {
		src := srcStruct{Name: "src名字", Tags: []string{"old"}}
		dst := dstStruct{Name: "dst名字", Tags: []string{"new"}}
...
func TestCoverByAssign(t *testing.T) {
	...
	src = structs.NewStruct(src).CoverByAssign(dst, "Name").Value()
	...
	t.Run("空白名单时什么都不覆盖", func(t *testing.T) {
		...
		src = structs.NewStruct(src).CoverByAssign(dst).Value()
