...
func Test1(t *testing.T) {
	t.Run("查看当前目录", func(t *testing.T) {
		t.Logf("当前目录：%s", APP.Dir.Rel().FullPath)
	})
}
...
func Test4(t *testing.T) {
	t.Run("删除单个目录", func(t *testing.T) {
		dir := APP.Dir.Rel(DirPath("test-a", "test-b", "test-c"))
...
func Test5(t *testing.T) {
	t.Run("删除多级目录", func(t *testing.T) {
		dir := APP.Dir.Rel(DirPath("test-a"))
...
func Test6(t *testing.T) {
	t.Run("列出当前目录下的所有文件和子目录", func(t *testing.T) {
		dir := APP.Dir.Rel(DirPath("test-a1", "test-a2", "test-a3"))
