...
func Test1(t *testing.T) {
	cg := NewCoroutineGroup[[]string]().
		SetBatches(3).
		SetCapacity(4).
		GO(func(batch, capacity uint) ResultImpl[[]string] {
			return ResultImpl[[]string]{
				Data:  []string{"轮数:", cast.ToString(batch), "次数:", cast.ToString(capacity)},
				Error: nil,
			}
		})

	for idx := range cg.GetResults() {
		t.Logf("%v", cg.GetResults()[idx])
	}
}
...
func Test2(t *testing.T) {
	cg := NewCoroutineGroup[[]string]().
		SetBatches(3).
		SetCapacity(4).
		GO(func(batch, capacity uint) (result ResultImpl[[]string]) {
			result = ResultImpl[[]string]{}
