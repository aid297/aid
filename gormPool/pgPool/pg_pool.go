...
type (
	PGPool interface {
		...
		SetSources(sources map[string]*gormPool.PGEndpoint)
		SetReplicas(replicas map[string]*gormPool.PGEndpoint)
		getRws() *gorm.DB
		GetConn() *gorm.DB
		Close() error
	}
	...
	PGPoolImpl struct {
		...
		mainConn     *gorm.DB
		sources      map[string]*gormPool.PGEndpoint
		replicas     map[string]*gormPool.PGEndpoint
		timezone     string
	}
)
...
func (my *PGPoolImpl) SetSources(sources map[string]*gormPool.PGEndpoint)   { my.sources = sources }
func (my *PGPoolImpl) SetReplicas(replicas map[string]*gormPool.PGEndpoint) { my.replicas = replicas }
