...
type (
	...
	Symmetric interface {
		...
		EncryptFile(plainFile, outFile string, asymmetric Asymmetric) error       // 加密文件（根据 Algorithm 选择 ECB/CBC）
		DecryptFile(cipherFile, outFile string, asymmetric Asymmetric) error      // 解密文件（根据 Algorithm 选择 ECB/CBC）
		EncryptLargeFile(plainFile, outFile string, asymmetric Asymmetric) error  // 加密大文件（根据 Algorithm 选择 ECB/CBC）
		DecryptLargeFile(cipherFile, outFile string, asymmetric Asymmetric) error // 解密大文件（根据 Algorithm 选择 ECB/CBC）
	}
)
