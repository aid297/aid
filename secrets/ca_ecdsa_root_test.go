...
func newRootCrt(t *testing.T) (*x509.Certificate, secrets.Semen) {
	t.Helper()

	var (
		err                              error
		caSem                            secrets.Semen
		caPriv                           secrets.SemenPriKey
		caPub                            secrets.SemenPubKey
		...
		caDER                            []byte
		caPrivFile, caPubFile, caCrtFile filesystems.Filesystem
		pemBlock                         *pem.Block
	...
	caPrivFile = filesystems.NewFile(filesystems.Rel(testPath, "ca.key"))
	if err = caPrivFile.Write(caPrivPEM, filesystems.Mode(0600), filesystems.Flag(os.O_CREATE|os.O_TRUNC|os.O_WRONLY)).GetError(); err != nil {
	...
	caPubFile = filesystems.NewFile(filesystems.Rel(testPath, "ca.pub"))
	if err = caPubFile.Write(caPubPEM, filesystems.Mode(0644), filesystems.Flag(os.O_CREATE|os.O_TRUNC|os.O_WRONLY)).GetError(); err != nil {
	...
	caCrtFile = filesystems.NewFile(filesystems.Rel(testPath, "ca.crt"))
	if err = caCrtFile.Write(caPEM, filesystems.Mode(0644), filesystems.Flag(os.O_CREATE|os.O_TRUNC|os.O_WRONLY)).GetError(); err != nil {
...
func newServerCrt(t *testing.T) (*x509.Certificate, secrets.Semen) {
	t.Helper()

	var (
		err                                                                 error
		serverSem, caSem                                                    secrets.Semen
		caPriv                                                              secrets.SemenPriKey
		serverPub                                                           secrets.SemenPubKey
		serverPrivPEM, serverPubPEM, serverPEM                              []byte
		serverPrivFile, serverPubFile, caPrivFile, caCrtFile, serverCrtFile filesystems.Filesystem
		ok                                                                  bool
	...
	// 获取 CA 根证书与私钥
	if caPrivFile = filesystems.NewFile(filesystems.Rel(testPath, "ca.key")); !caPrivFile.GetExist() {
		t.Fatalf("[CA 私钥]文件不存在：%v", err)
	}
	if caPrivPEM, err = caPrivFile.Read(filesystems.Mode(0600)); err != nil {
	...
	if caCrtFile = filesystems.NewFile(filesystems.Rel(testPath, "ca.crt")); !caCrtFile.GetExist() {
		t.Fatalf("[CA 证书]文件不存在：%v", err)
	}
	if caCrtPEM, err = caCrtFile.Read(filesystems.Mode(0644)); err != nil {
	...
	serverPrivFile = filesystems.NewFile(filesystems.Rel(testPath, "server.key"))
	if err = serverPrivFile.Write(serverPrivPEM, filesystems.Mode(0600), filesystems.Flag(os.O_CREATE|os.O_TRUNC|os.O_WRONLY)).GetError(); err != nil {
	...
	serverPubFile = filesystems.NewFile(filesystems.Rel(testPath, "server.pub"))
	if err = serverPubFile.Write(serverPubPEM, filesystems.Mode(0644), filesystems.Flag(os.O_CREATE|os.O_TRUNC|os.O_WRONLY)).GetError(); err != nil {
	...
	serverCrtFile = filesystems.NewFile(filesystems.Rel(testPath, "server.crt"))
	if err = serverCrtFile.Write(serverPEM, filesystems.Mode(0644), filesystems.Flag(os.O_CREATE|os.O_TRUNC|os.O_WRONLY)).GetError(); err != nil {
...
func newClientCrt(t *testing.T) (*x509.Certificate, secrets.Semen) {
	t.Helper()

	var (
		err                         error
		ok                          bool
		clientSem, caSem            secrets.Semen
		clientPub                   secrets.SemenPubKey
		clientPrivPEM, clientPubPEM []byte
		caPrivFile, clientPrivFile, clientPubFile,
		caCrtFile, clientCrtFile filesystems.Filesystem
		caCrtPEM, caPrivPEM, clientDER []byte
		caCrtBlock                     *pem.Block
		caCrt, clientTpl, clientCrt    *x509.Certificate
		caPriv, clientPriv             secrets.SemenPriKey
		sn                             *big.Int
		now                            time.Time
	...
	clientPrivFile = filesystems.NewFile(filesystems.Rel(testPath, "client.key"))
	if err = clientPrivFile.Write(clientPrivPEM, filesystems.Mode(0600), filesystems.Flag(os.O_CREATE|os.O_TRUNC|os.O_WRONLY)).GetError(); err != nil {
	...
	clientPubFile = filesystems.NewFile(filesystems.Rel(testPath, "client.pub"))
	if err = clientPubFile.Write(clientPubPEM, filesystems.Mode(0644), filesystems.Flag(os.O_CREATE|os.O_TRUNC|os.O_WRONLY)).GetError(); err != nil {
	...
	clientCrtFile = filesystems.NewFile(filesystems.Rel(testPath, "client.crt"))
	if err = clientCrtFile.Write(
		pem.EncodeToMemory(&pem.Block{Type: "CERTIFICATE", Bytes: clientDER}),
		filesystems.Mode(0644),
		filesystems.Flag(os.O_CREATE|os.O_TRUNC|os.O_WRONLY),
	).GetError(); err != nil {
