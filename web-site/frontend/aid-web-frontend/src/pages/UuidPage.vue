...
onMounted(() => {
    axios.post('/uuid/versions').then(res => {
        for (const [k, v] of Object.entries(res.data.content.versions)) versions.value.push({ label: k, value: v });
    });
});
...
const onSubmit = async () => {
    ...
    console.log('测试', (await axios.post('/uuid/generate', {
        body: {
            number: parseInt(number.value),
            noSubsTractKey: noSubsTractKey.value,
            isUpper: isUpper.value,
    ...
    rows.value = (await axios.post('/uuid/generate', {
        body: {
            number: parseInt(number.value),
            noSubsTractKey: noSubsTractKey.value,
            isUpper: isUpper.value,
