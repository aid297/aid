...
// List 留言板服务：获取信息列表
func (*MessageBoardService) List() (data []kernal.Row, err error) { return global.DB.Find() }

// Store 留言板服务：保存信息
func (*MessageBoardService) Store(form *request.MessageBoardStoreRequest) (err error) {
	rows := []kernal.Row{{"content": form.Content}}
...
func (*MessageBoardService) Destroy(form *request.MessageBoardDestroyRequest) (err error) {
	_, err = global.DB.RemoveByCondition(kernal.QueryCondition{
		Field:    "id",
