...
```go
   ...
   import (
   	...
   	"github.com/aid297/aid/v3/anyMap"
   )
   
   func main() {
   	m1 := anyMaps.New(anyMaps.Map(map[string]string{"姓名": "张三", "年龄": "18"}))
   	Println(m1.ToString()) //map[姓名:张三 年龄:18]
   
   	m2 := anyMaps.New(anyMaps.Cap[string, int](10))
   	m2.SetAttrs(anyMaps.Map(map[string]int{"张三": 18, "李四": 19, "王五": 20, "赵六": 21}))
   	Println(m2.ToString()) // map[张三:18 李四:19 王五:20 赵六:21]
   
   	m3 := anyMaps.New(anyMaps.Cap[string, bool](5))
...
```go
   ...
   func main() {
   	m1 := anyMaps.New(anyMaps.Map(map[string]any{"张三": 18, "李四": 19, "王五": 20, "赵六": 21}))
...
```go
   ...
   func main() {
   	Println(anyMaps.New(anyMaps.Map(map[string]any{})).IsEmpty())          // true
   	Println(anyMaps.New(anyMaps.Map(map[string]any{"a": 1})).IsEmpty())    // false
   	Println(anyMaps.New(anyMaps.Map(map[string]any{"a": 1})).IsNotEmpty()) // true
   }
```
---
...
```go
   ...
   func main() {
   	m1 := anyMaps.New(anyMaps.Map(map[string]any{"a": 1, "b": 2, "c": 3}))
...
```go
   ...
   func main() {
     ...
     // 注意：从这个案例中可以看出，anyMaps.Map初始化这种方式并不能保证 `Key` 的顺序
   }
```
---
...
```go
   ...
   func main() {
   	m1 := anyMaps.New(anyMaps.Map(map[string]any{"a": 1, "b": 2, "c": 3, "d": 0, "e": nil, "f": nil, "g": struct{}{}}))
   	Printf("length: %d\n", m1.Length())                        // length: 7
...
```go
    ...
    func main() {
    	m1 := anyMaps.New(anyMaps.Map(map[string]uint8{"张三": 40, "李四": 50, "王五": 60, "赵六": 70}))
...
```go
    ...
    func main() {
    	m2 := anyMaps.Cast(
    		anyMaps.New(anyMaps.Map(map[string]uint8{"张三": 40, "李四": 50, "王五": 60, "赵六": 70})),
...
```go
    ...
    func main() {
    	...
    	r, e := anyMaps.StructToOther[User, map[string]any](u)
    	if e != nil {
    		panic(e)
    	}
