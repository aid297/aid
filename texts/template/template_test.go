...
func TestStruct_Basic(t *testing.T) {
	...
	got := template.TemplaterImpl{}.New(tpl, template.Struct(Data{Name: "Tom", Age: 18})).String()
...
func TestStruct_Pointer(t *testing.T) {
	...
	got := template.TemplaterImpl{}.New("Hi {{name}}", template.Struct(data)).String()
	if got != "Hi Jerry" {
...
func TestStruct_MultiplePlaceholders(t *testing.T) {
	...
	got := template.TemplaterImpl{}.New(tpl, template.Struct(Data{First: "John", Last: "Doe"})).String()
...
func TestStruct_FieldWithoutTag(t *testing.T) {
	...
	got := template.TemplaterImpl{}.New("{{name}}", template.Struct(Data{Name: "Alice", Ignored: "x"})).String()
	if got != "Alice" {
...
func TestStruct_TagDash(t *testing.T) {
	...
	got := template.TemplaterImpl{}.New("{{Name}}", template.Struct(Data{Name: "Skip"})).String()
...
func TestMap_Basic(t *testing.T) {
	...
	got := template.TemplaterImpl{}.New(tpl, template.Map(data)).String()
...
func TestMap_Pointer(t *testing.T) {
	...
	got := template.TemplaterImpl{}.New("{{key}}", template.Struct(&data)).String()
	if got != "value" {
...
func TestMap_Empty(t *testing.T) {
	got := template.TemplaterImpl{}.New("hello {{name}}", template.Map(map[string]any{})).String()
	if got != "hello {{name}}" {
...
func TestNoDataSource(t *testing.T) {
	// 未提供数据源时，占位符保留原样
	got := template.TemplaterImpl{}.New("hello {{name}}").String()
	if got != "hello {{name}}" {
...
func TestEmptyContent(t *testing.T) {
	got := template.TemplaterImpl{}.New("", template.Struct(struct {
		Name string `template:"name"`
	}{Name: "x"})).String()
	if got != "" {
...
func TestBytes(t *testing.T) {
	...
	got := template.TemplaterImpl{}.New("{{name}}", template.Struct(Data{Name: "abc"})).Bytes()
	if string(got) != "abc" {
...
func TestContent(t *testing.T) {
	tpl := "hello {{name}}"
	ins := template.TemplaterImpl{}.New(tpl, template.Struct(struct {
		Name string `template:"name"`
	}{Name: "world"}))
	if ins.Content() != tpl {
...
func TestError_NoDataSource(t *testing.T) {
	...
	ins := template.TemplaterImpl{}.New("{{name}}")
...
func TestError_InvalidType(t *testing.T) {
	// 传入 int 类型
	ins := template.TemplaterImpl{}.New("{{name}}", template.Struct(42))
	if ins.Error() == nil {
...
func TestError_NonStructNonMap(t *testing.T) {
	// 传入 string 类型
	ins := template.TemplaterImpl{}.New("{{name}}", template.Struct("just a string"))
	if ins.Error() == nil {
...
func TestPlaceholderNoMatch(t *testing.T) {
	...
	got := template.TemplaterImpl{}.New("{{name}} is {{age}}", template.Struct(Data{Name: "Alice"})).String()
...
func TestRender_NoPlaceholders(t *testing.T) {
	...
	got := template.TemplaterImpl{}.New("plain text", template.Struct(Data{Name: "x"})).String()
	if got != "plain text" {
...
func TestStruct_BooleanAndFloat(t *testing.T) {
	...
	got := template.TemplaterImpl{}.New("active={{active}}, score={{score}}",
