...
type (
	...
	HTTPClient interface {
		...
		setEncryptor(symmetricEncryptor secrets.Symmetric)
	...
	HTTPClientImpl struct {
		...
		autoCopy           bool
		lock               sync.RWMutex
		symmetricEncryptor secrets.Symmetric
		compressor         compressions.Compressor
		filePath           string
		fileSize           int64
		isChunked          bool
...
func (my *HTTPClientImpl) setBodyFile(filename string, goroutineCount uint64) (err error) {
	...
	if goroutineCount > 1 && size > fileSplitSize {
		...
		if err = file.Close(); err != nil {
			debugLogs.Error("[HTTP Client] 关闭文件错误: %v", err)
		}
...
func (my *HTTPClientImpl) setEncryptor(symmetricEncryptor secrets.Symmetric) {
...
func (my *HTTPClientImpl) GetStatusCode() int {
	return operations.NewTernary(operations.TrueFn(func() int { return my.GetRawResponse().StatusCode })).GetByValue(my.GetRawResponse() != nil)
}

func (my *HTTPClientImpl) GetStatus() string {
	return operations.NewTernary(operations.TrueFn(func() string { return my.GetRawResponse().Status })).GetByValue(my.GetRawResponse() != nil)
}
