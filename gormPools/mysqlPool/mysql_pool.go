...
type (
	MySQLPool interface {
		...
		SetHost(host string)
		SetPort(port uint16)
		SetSources(sources map[string]gormPools.MySQLEndpoint)
		SetReplicas(replicas map[string]gormPools.MySQLEndpoint)
	...
	MySQLPoolImpl struct {
		...
		database           string
		charset            string
		sources            map[string]gormPools.MySQLEndpoint
		replicas           map[string]gormPools.MySQLEndpoint
...
func New(database, charset string, rws bool, attrs ...MySQLPoolAttr) (MySQLPool, error) {
	...
	// 配置主库
	ins.mainDsn = &gormPools.DSN{Name: "main", Content: fmt.Sprintf(
...
func (my *MySQLPoolImpl) SetSources(sources map[string]gormPools.MySQLEndpoint) { my.sources = sources }
func (my *MySQLPoolImpl) SetReplicas(replicas map[string]gormPools.MySQLEndpoint) {
